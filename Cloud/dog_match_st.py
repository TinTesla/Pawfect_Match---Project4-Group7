...
st.write("""
# Simple Dog Breed and Group Match Prediction App

This app predicts the pawfect ** Dog Match ** for you!
""")
...
def user_input_features_group():
    height = []
    height = st.slider('Minimum, Maximum Height', 10.0, 100.0, (20.0,80.0))
    min_height = height[0]
    max_height = height[1]

    weight = []
    weight = st.slider('Minimum Weight, Maximum', 0.0, 79.0, (3.0,30.0))
    min_weight = weight[0]
    max_weight = weight[1]

    expectancy = []
    expectancy = st.slider('Minimum, Maximum expactancy', 0.0, 16.0, (3.0,12.0))
    min_expectancy = expectancy[0]
    max_expectancy = expectancy[1]

    popularity = st.slider('How popular your would like your freind to be', 0, 148, 75)
    grooming_frequency_value = st.slider('grooming_frequency_value', 0.0,1.0,0.0,step=0.20)
    shedding_value = st.slider('shedding_value',0.0,1.0,0.0,step=0.20)
    energy_level_value = st.slider('energy_level_value', 0.0,1.0, 0.0,step=0.20)
    trainability_value = st.slider('trainability_value', 0.0,1.0,0.0,step=0.20)
    demeanor_value = st.slider('demeanor_value',  0.0,1.0,0.0,step=0.20)
...
def group_prediction(user_input_df):
    ...
    # Make predictions
    predicted_group = loaded_model.predict(input_array)[0]
...
def main():
    ...
    user_input = user_input_features_group()
    pred_group = group_prediction(user_input_df=user_input)
    st.write("Predicted Class:", dog_map_group_df['group'].iloc[pred_group])

dog_map_group_df = pd.read_csv('../sourcedata/dog_group_mapping.csv')
user_input = user_input_features_group()
pred_group = group_prediction(user_input)
st.write("Predicted Group:", dog_map_group_df['group'].iloc[pred_group])
pred_dog_group = st.button('Find my predicted Group')
